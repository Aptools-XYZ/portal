...
const ClaimCompV1: React.FC = () => {
    ...
    (async () => {
      ...
      try {
        ...
        const store = await aptosClient.getAccountResource(
          process.env.NEXT_PUBLIC_AIRDROP_RESOURCE_ADDRESS!,
          `${process.env
            .NEXT_PUBLIC_FACTORY_V1_ADDRESS!}::airdropper::Store<${currency}>`
        );
        ...
        try {
          const res = await aptosClient.getTableItem(handle, {
            ...
            value_type: `${process.env
              .NEXT_PUBLIC_FACTORY_V1_ADDRESS!}::airdropper::Airdrop<${currency}>`,
            key: index,
          });
          const airdrop = {
            id: index,
            ...
            reserve: { value: res.reserve.value },
            owner: res.owner,
  ...
  async function claim() {
    ...
    try {
      const payload = makeAirdropClaimPayload(selectedCoin?.type, +index!);
  ...
  function renderInner() {
    return (
      <div className="py-6 pl-6 xs:py-4 xs:pl-2">
        <h2 className="text-2xl font-bold">Claim Airdrop on Aptos (v1)</h2>
